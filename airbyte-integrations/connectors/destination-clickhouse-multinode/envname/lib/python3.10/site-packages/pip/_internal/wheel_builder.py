...
def _should_build(
    ...
    if not is_wheel_installed():
        ...
        logger.info(
            "Using legacy 'setup.py install' for %s, " "since package 'wheel' is not installed.",
            req.name,
        )
        return False
...
def _verify_one(req, wheel_path):
    ...
    if canonicalize_name(w.name) != canonical_name:
        raise InvalidWheelFilename(
            "Wheel has unexpected file name: expected {!r}, " "got {!r}".format(canonical_name, w.name),
        )
    dist = get_wheel_distribution(wheel_path, canonical_name)
    dist_verstr = str(dist.version)
    if canonicalize_version(dist_verstr) != canonicalize_version(w.version):
        raise InvalidWheelFilename(
            "Wheel has unexpected file name: expected {!r}, " "got {!r}".format(dist_verstr, w.version),
        )
...
def _build_one(
    ...
    with req.build_env:
        wheel_path = _build_one_inside_env(req, output_dir, build_options, global_options)
...
def build(
    ...
    with indent_log():
        ...
        for req in requirements:
            ...
            wheel_file = _build_one(req, cache_dir, verify, build_options, global_options)
            if wheel_file:
