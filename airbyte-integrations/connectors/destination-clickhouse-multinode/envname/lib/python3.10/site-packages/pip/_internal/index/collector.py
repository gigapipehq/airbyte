...
from typing import Callable, Iterable, List, MutableMapping, NamedTuple, Optional, Sequence, Union
...
def _handle_get_page_fail(link: Link, reason: Union[str, Exception], meth: Optional[Callable[..., None]] = None) -> None:
    if meth is None:
        meth = logger.debug
...
def _make_html_page(response: Response, cache_link_parsing: bool = True) -> HTMLPage:
    ...
    return HTMLPage(response.content, encoding=encoding, url=response.url, cache_link_parsing=cache_link_parsing)
...
def _get_html_page(link: Link, session: Optional[PipSession] = None) -> Optional["HTMLPage"]:
    ...
    try:
        ...
        logger.warning(
            "Skipping page %s because the %s request got Content-Type: %s." "The only supported Content-Type is text/html",
            link,
            exc.request_desc,
            exc.content_type,
        )
