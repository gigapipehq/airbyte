...
from pip._internal.utils.misc import ask, dist_in_usersite, dist_is_local, egg_link_path, is_local, normalize_path, renames, rmtree
...
def uninstallation_paths(dist: Distribution) -> Iterator[str]:
    ...
    try:
        ...
        try:
            ...
            msg += " You might be able to recover from this via: " "'pip install --force-reinstall --no-deps {}'.".format(dep)
        else:
...
class UninstallPathSet:
    ...
    def from_dist(cls, dist: Distribution) -> "UninstallPathSet":
        ...
        if dist_path in {p for p in {sysconfig.get_path("stdlib"), sysconfig.get_path("platstdlib")} if p}:
            logger.info(
        ...
        if egg_info_exists and dist.egg_info.endswith(".egg-info") and not dist.egg_info.endswith(develop_egg_link_egg_info):
            ...
            assert link_pointer == dist.location, "Egg-link {} does not match installed location of {} " "(at {})".format(
