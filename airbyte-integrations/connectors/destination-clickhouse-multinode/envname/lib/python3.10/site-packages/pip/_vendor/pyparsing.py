...
class ParseBaseException(Exception):
    ...
    def __str__(self):
        if self.pstr:
            ...
            foundstr = ""
        return "%s%s  (at char %d), (line:%d, col:%d)" % (self.msg, foundstr, self.loc, self.lineno, self.column)
...
class ParseResults(object):
                    ...
                    if isinstance(vv, ParseResults):
                        out.append(
                            ...
                            % (
                                ...
                                indent,
                                ("  " * (_depth + 1)),
                                vv.dump(indent=indent, full=full, include_list=include_list, _depth=_depth + 1),
                            )
                        )
                    else:
                        out.append("\n%s%s[%d]:\n%s%s%s" % (indent, ("  " * (_depth)), i, indent, ("  " * (_depth + 1)), _ustr(vv)))
    ...
    def __getstate__(self):
        return (
            self.__toklist,
            (self.__tokdict.copy(), self.__parent is not None and self.__parent() or None, self.__accumNames, self.__name),
        )
...
class ParserElement(object):
    ...
    def __getitem__(self, key):
        ...
        if len(key) > 2:
            warnings.warn(
                "only 1 or 2 index arguments supported ({0}{1})".format(key[:5], "... [{0}]".format(len(key)) if len(key) > 5 else "")
            )
...
class Regex(Token):
    ...
    def sub(self, repl):
        ...
        """
        if self.asGroupList:
            warnings.warn("cannot use sub() with Regex(asGroupList=True)", SyntaxWarning, stacklevel=2)
            raise SyntaxError()

        if self.asMatch and callable(repl):
            warnings.warn("cannot use sub() with a callable with Regex(asMatch=True)", SyntaxWarning, stacklevel=2)
            raise SyntaxError()
    ...
    def __init__(
        ...
        if len(self.endQuoteChar) > 1:
            self.pattern += (
                "|(?:"
                + ")|(?:".join(
                    "%s[^%s]" % (re.escape(self.endQuoteChar[:i]), _escapeRegexRangeChars(self.endQuoteChar[i]))
                ...
                if isinstance(e, ParserElement) and e.resultsName:
                    warnings.warn(
                        ...
                        "collides with {3!r} on contained expression".format(
                            "warn_ungrouped_named_tokens_in_collection", name, type(self).__name__, e.resultsName
                        ),
                        stacklevel=3,
                    )
...
_reBracketExpr = (
    Literal("[") + Optional("^").setResultsName("negate") + Group(OneOrMore(_charRange | _singleChar)).setResultsName("body") + "]"
)
        ...
        if isinstance(opener, basestring) and isinstance(closer, basestring):
            if len(opener) == 1 and len(closer) == 1:
                if ignoreExpr is not None:
                    content = Combine(
                        OneOrMore(~ignoreExpr + CharsNotIn(opener + closer + ParserElement.DEFAULT_WHITE_CHARS, exact=1))
                    ).setParseAction(lambda t: t[0].strip())
                else:
                    content = empty.copy() + CharsNotIn(opener + closer + ParserElement.DEFAULT_WHITE_CHARS).setParseAction(
                        lambda t: t[0].strip()
                    )
            else:
                if ignoreExpr is not None:
...
_commasepitem = (
    Combine(OneOrMore(Word(printables, excludeChars=",") + Optional(Word(" \t") + ~Literal(",") + ~LineEnd())))
    .streamline()
    .setName("commaItem")
)
...
class pyparsing_common:
    ...
    _short_ipv6_address = (
        Optional(_ipv6_part + (":" + _ipv6_part) * (0, 6)) + "::" + Optional(_ipv6_part + (":" + _ipv6_part) * (0, 6))
    ...
    _commasepitem = (
        Combine(OneOrMore(~Literal(",") + ~LineEnd() + Word(printables, excludeChars=",") + Optional(White(" \t"))))
        .streamline()
        .setName("commaItem")
    )
...
pyparsing_unicode.Japanese._ranges = (
    pyparsing_unicode.Japanese.Kanji._ranges + pyparsing_unicode.Japanese.Hiragana._ranges + pyparsing_unicode.Japanese.Katakana._ranges
)
