...
def _makeTags(tagStr, xml):
    ...
    if xml:
        ...
        openTag = (
            ...
            + Optional("/", default=[False]).setResultsName("empty").setParseAction(lambda s, l, t: t[0] == "/")
            + Suppress(">")
        )
    else:
...
class pyparsing_common:
    ...
    sci_real = (
        Regex(r"[+-]?\d+([eE][+-]?\d+|\.\d*([eE][+-]?\d+)?)").setName("real number with scientific notation").setParseAction(convertToFloat)
    )
