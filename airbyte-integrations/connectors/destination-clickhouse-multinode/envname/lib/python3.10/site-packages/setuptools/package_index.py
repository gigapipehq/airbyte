...
def interpret_distro_name(location, basename, metadata, py_version=None, precedence=SOURCE_DIST, platform=None):
    ...
    for p in range(1, len(parts) + 1):
        yield Distribution(
            location, metadata, "-".join(parts[:p]), "-".join(parts[p:]), py_version=py_version, precedence=precedence, platform=platform
        )
    ...
    def check_hash(self, checker, filename, tfp):
        ...
        if not checker.is_valid():
            ...
            raise DistutilsError(
                "%s validation failed for %s; " "possible download problem?" % (checker.hash.name, os.path.basename(filename))
            )
