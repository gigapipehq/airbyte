...
def _msvc14_find_vc2015():
    ...
    try:
        key = winreg.OpenKey(
            winreg.HKEY_LOCAL_MACHINE, r"Software\Microsoft\VisualStudio\SxS\VC7", 0, winreg.KEY_READ | winreg.KEY_WOW64_32KEY
        )
    except OSError:
        return None, None
...
def _augment_exception(exc, version, arch=""):
    ...
    if "vcvarsall" in message.lower() or "visual c" in message.lower():
        ...
        if version == 9.0:
            ...
            message += ' Get it with "Microsoft C++ Build Tools": ' r"https://visualstudio.microsoft.com" r"/visual-cpp-build-tools/"
...
class PlatformInfo:
    ...
    def target_dir(self, hidex86=False, x64=False):
        ...
        """
        return (
            "" if (self.target_cpu == "x86" and hidex86) else r"\x64" if (self.target_cpu == "amd64" and x64) else r"\%s" % self.target_cpu
        )
...
class EnvironmentInfo:
    ...
    def VCIncludes(self):
        ...
        list of str
            paths
        """
        return [join(self.si.VCInstallDir, "Include"), join(self.si.VCInstallDir, r"ATLMFC\Include")]
    ...
    def return_env(self, exists=True):
        ...
        """
        env = dict(
            include=self._build_paths("include", [self.VCIncludes, self.OSIncludes, self.UCRTIncludes, self.NetFxSDKIncludes], exists),
            lib=self._build_paths(
                "lib", [self.VCLibraries, self.OSLibraries, self.FxTools, self.UCRTLibraries, self.NetFxSDKLibraries], exists
            ),
            libpath=self._build_paths("libpath", [self.VCLibraries, self.FxTools, self.VCStoreRefs, self.OSLibpath], exists),
